...
import { ProgramRowItem } from "@/components/dashboard/programs/ProgramRowItem";
import { ProgramsCategoriesPanel } from "@/components/dashboard/programs/ProgramsCategoriesPanel";

type Tab =
  | { key: "all"; label: string; kind: "all" }
  | { key: "settings"; label: string; kind: "settings" }
  | { key: string; label: string; kind: "category"; category: ProgramCategoryRow };
...
function ProgramsDashboardClient({
  items,
  categories,
  programsPageEditorHref,
}: {
  items: ProgramRow[];
  categories: ProgramCategoryRow[];
  programsPageEditorHref: string | null;
}) {
  ...
  const categoryById = useMemo(() => {
    const m = new Map<string, ProgramCategoryRow>();
  ...
  function setTab(next: string) {
    ...
    router.push(`/dashboard/programs${q.toString() ? `?${q}` : ""}`);
  }
  ...
  const newProgramHref = activeCategory
    ? `/dashboard/programs/new?category=${encodeURIComponent(activeCategory.slug)}`
    : "/dashboard/programs/new";

  const newCtaLabel = activeCategory
    ? `New ${activeCategory.label.toLowerCase()} article`
    : "New program";
  ...
  return (
    <div className="space-y-10">
          ...
          <div className="relative flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 gap-5">
              <div className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-[#7A1515] text-white">
                <Grid3x3 className="size-7" strokeWidth={1.75} aria-hidden />
              </div>
              <div className="min-w-0 space-y-2">
                <div>
                  ...
                  <p className="mt-1 max-w-xl text-sm leading-relaxed text-stone-500">
                    Activities, projects and stories for each program area. Articles support rich text,
                    images, files and links — broken into categories so each pillar has its own tab.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-medium text-stone-500">
                  ...
                  >
                    Open /programs <ArrowUpRight className="size-3.5 opacity-70" aria-hidden />
                  </a>
                  {programsPageEditorHref ? (
                    <Link
                      href={programsPageEditorHref}
      ...
      {activeTab.kind === "settings" ? (
        <ProgramsCategoriesPanel
          categories={sortedCategories}
          countsBySlug={counts.byCategory}
        />
      ) : (
        <ProgramsListPanel
      ...
      <ConfirmDialog
        ...
        description="This removes the program article. Cover images and inline media stay in the Media Library unless deleted separately."
...
export default ProgramsDashboardClient;
...
function ProgramsListPanel({
  ...
  activeCategory: ProgramCategoryRow | null;
  items: ProgramRow[];
  categoryBySlug: Map<string, ProgramCategoryRow>;
  allCategoryById: Map<string, ProgramCategoryRow>;
        ...
        <div className="mx-auto flex max-w-md flex-col items-center">
          <div className="mb-6 flex size-24 items-center justify-center rounded-full bg-stone-100">
            {activeCategory ? (
              ...
              <Grid3x3 className="size-10 text-[#7A1515]/45" strokeWidth={1.25} aria-hidden />
            )}
          </div>
          <h3 className="text-lg font-semibold text-stone-900">
            {activeCategory
              ? `No ${(activeCategory.plural_label || activeCategory.label).toLowerCase()} articles yet`
              : "No programs yet"}
          </h3>
          <p className="mt-2 max-w-sm text-sm leading-relaxed text-stone-500">
            {activeCategory?.description ||
              "Create your first program article to surface it on /programs once published."}
          </p>
          <Link
            href={newProgramHref}
      ...
      <div>
        <div className="mb-5 flex flex-wrap items-baseline justify-between gap-4">
          <div className="flex items-center gap-3">
            <ProgramCategoryIcon icon={activeCategory.icon} accent={activeCategory.accent} size={28} />
            <div>
  ...
  // "All" tab: group by category for orientation
        ...
        return (
          <div key={slug}>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                {cat ? (
                  <ProgramCategoryIcon icon={cat.icon} accent={cat.accent} size={26} />
                ) : null}
              ...
              {cat ? (
                <Link
                  href={`/dashboard/programs/new?category=${encodeURIComponent(cat.slug)}`}
                  className="text-[11px] font-semibold text-[#7A1515] hover:underline"
                >
                  + Add {cat.label.toLowerCase()} article
                </Link>
              ) : null}
            </div>
